AFRAME.registerComponent('zip-loader', {
  ...
  fetchZip: function (version) {
    ...
    if (this.data.directDownload.indexOf('assets/songs/') === 0 &&
        !this.data.directDownload.endsWith('.zip')) {
      this.fetchBundledSong(version);
      return;
    }
  ...
  fetchBundledSong: function (version) {
    const base = this.data.directDownload;
    const difficulties = ['Normal', 'Hard', 'Expert', 'ExpertPlus'];
    const requests = difficulties.map(difficulty => {
      return fetch(`${base}/${difficulty}.dat`)
        .then(response => {
          if (!response.ok) { throw new Error(`HTTP ${response.status}`); }
          return response.json();
        })
        .then(beats => {
          beats._beatsPerMinute = this.data.bpm;
          return {
    ...
    Promise.all(requests).then(results => {
      ...
      results.forEach(result => {
        data.beats[result.id] = result.beats;
      });

      this.cachedVersion = version;
      this.cachedZip = data;
      this.el.emit('ziploaderend', data, false);
    }).catch(err => {
      console.error('[bundled-song] Failed to load default song.', err);
      this.cachedZip = null;
      this.el.emit('songloaderror');
    });
  },
