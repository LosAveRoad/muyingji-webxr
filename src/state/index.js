...
const badSongs = {};
const DEFAULT_CHALLENGE = convertBeatmap(require('../lib/search.json')[0]);
DEFAULT_CHALLENGE.directDownload = 'assets/songs/shut-up-and-dance';
DEFAULT_CHALLENGE.metadata.bpm = 143;
DEFAULT_CHALLENGE.metadata.duration = 60;
...
AFRAME.registerState({
  ...
  computeState: state => {
    ...
    state.score.active = false;
  }
});

function prepareDefaultChallenge (state) {
  Object.assign(state.menuSelectedChallenge, DEFAULT_CHALLENGE);

  state.menuDifficulties.length = 0;
  state.menuDifficultiesIds.length = 0;

  const characteristics = JSON.parse(DEFAULT_CHALLENGE.metadata.characteristics);
  const standard = characteristics.Standard || {};
  Object.keys(standard).forEach(difficulty => {
    if (!standard[difficulty]) { return; }
    const option = {
      id: `Standard-${difficulty}`,
      filename: `${difficulty}Standard`,
      difficultyName: difficultyMap[difficulty],
      renderName: difficultyMap[difficulty],
      beatmapCharacteristic: 'Standard',
      difficulty: difficulty
    };
    state.menuDifficulties.push(option);
    state.menuDifficultiesIds.push(option.id);
  });

  const selected = state.menuDifficulties.find(item => item.difficulty === 'Normal') ||
    state.menuDifficulties[0];
  state.menuSelectedChallenge.difficulty = selected.difficulty;
  state.menuSelectedChallenge.beatmapCharacteristic = selected.beatmapCharacteristic;
  state.menuSelectedChallenge.difficultyId = selected.id;
  state.menuSelectedChallenge.image = state.menuSelectedChallenge.coverURL;
